...
class UserTree extends Tree {
	...
	/**
	 * @var string
	 * TODO currently redundant with $expertUsernameAttr. fix when core properly distinguishes uid and username
	 */
	...
	public function __construct(array $data) {
		...
		$this->filterObjectclass =     isset($data['filterObjectclass'])     ?         $data['filterObjectclass']          : ['person'];
		...
		if ($this->loginFilterMode === self::LOGIN_FILTER_MODE_AUTO) {
			$this->loginFilter = "(&({$this->filter})({$this->usernameAttribute}=%uid))";
		} else {
