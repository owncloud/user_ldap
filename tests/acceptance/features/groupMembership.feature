...
So that I only need to configure group membership once
	...
	Scenario: adding a new user to a group after a folder was shared with that group
		...
		And the folder "simple-folder (2)" should be marked as shared with "grp1" by "User One"
	...
	Scenario: search for groups by alternative attribute
		...
		Then all users and groups that contain the string "grp1" in their name should be listed in the autocomplete list
