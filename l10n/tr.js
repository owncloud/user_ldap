OC.L10N.register(
    ...
    "Group-Member algorithm" : "Grup-Üye algoritması",
    "Algorithm to be used when searching for members in a group. In case of doubt, \"%s\" is the safe choice. Note that you're responsible of fulfilling the restrictions in order to use any algorithm; the app will behave erratically if the restrictions aren't fulfilled" : "Bir gruptaki üyeleri ararken kullanılacak algoritma. Şüphe durumunda, \"%s\" güvenli seçimdir. Herhangi bir algoritmayı kullanmak için kısıtlamaları yerine getirmekten sorumlu olduğunuzu unutmayın; kısıtlamalar yerine getirilmezse uygulama kararsız davranır",
    "Scan the group looking for users. This is the basic method and works without restrictions. It supports nested groups" : "Kullanıcı arayan grubu tarayın. Bu temel yöntemdir ve kısıtlama olmaksızın çalışır. Yuvalanmış grupları destekler",
    "Search the members using the \"memberOf\" attribute. It requires support for the \"memberOf\" attribute, and it doesn't work with nested groups" : "\"memberOf\" özniteliğini kullanarak üyeleri arayın. \"memberOf\" özniteliği için destek gerektirir ve iç içe gruplarla çalışmaz",
    "Search the members using the \"memberOf\" attribute in a recursive way. It requires support for the \"memberOf\" attribute and support for the \"LDAP_MATCHING_RULE_IN_CHAIN\" operator, it works with nested groups. Its main target is AD servers. NOTE: This algorithm only works with members explicitly listed in the group; AD's primary group, for example, won't be taken into account." : "\"memberOf\" özniteliğini kullanarak üyeleri yinelemeli bir şekilde arayın. \"memberOf\" niteliği için destek ve \"LDAP_MATCHING_RULE_IN_CHAIN\" operatörü için destek gerektirir, iç içe gruplarla çalışır. Ana hedefi AD sunucularıdır. NOT: Bu algoritma yalnızca grupta açıkça listelenen üyelerle çalışır; Örneğin, AD'nin birincil grubu dikkate alınmayacaktır.",
    ...
    "By default the internal username will be created from the UUID attribute. It makes sure that the username is unique and characters do not need to be converted. The internal username has the restriction that only these characters are allowed: [ a-zA-Z0-9+_.@- ].  Other characters are replaced with their ASCII correspondence or simply omitted. On collisions a number will be added/increased. The internal username is used to identify a user internally. It is also the default name for the user home folder. It is also a part of remote URLs, for instance for all *DAV services. With this setting, the default behavior can be overridden. To do so, enter the user display name attribute in the following field. Leave it empty for default behavior. Changes will have effect only on newly mapped (added) LDAP users." : "Varsayılan olarak dahili kullanıcı adı UUID özelliğinden oluşturulacaktır. Kullanıcı adının benzersiz olmasını ve karakterlerin dönüştürülmesine gerek olmamasını sağlar. Dahili kullanıcı adı, yalnızca şu karakterlere izin verilmesi kısıtlamasına sahiptir: [ a-zA-Z0-9+_.@- ]. Diğer karakterler ASCII karşılıklarıyla değiştirilir veya basitçe atlanır. Çarpışmalarda bir sayı eklenecek/arttırılacaktır. Dahili kullanıcı adı, bir kullanıcıyı dahili olarak tanımlamak için kullanılır. Ayrıca, kullanıcı ana klasörü için varsayılan addır. Ayrıca, örneğin tüm *DAV hizmetleri için uzak URL'lerin bir parçasıdır. Bu ayarla, varsayılan davranış geçersiz kılınabilir. Bunu yapmak için, aşağıdaki alana kullanıcı görünen adı özniteliğini girin. Varsayılan davranış için boş bırakın. Değişiklikler yalnızca yeni eşlenen (eklenen) LDAP kullanıcıları üzerinde etkili olacaktır.",
    "Internal Username Attribute:" : "Dahili Kullanıcı Adı Özniteliği:",
    "Internal Groupname" : "Dahili Grup adı",
    "The internal groupname is used to uniquely identify the group. It has the same restrictions as the internal username, in particular, the group name must be immutable and unique. By default, the UUID will be used. This internal groupname won't likely by visible because a displayname attribute is intended to be used to show the group." : "Dahili grup adı, grubu benzersiz bir şekilde tanımlamak için kullanılır. Dahili kullanıcı adıyla aynı kısıtlamalara sahiptir, özellikle grup adı değişmez ve benzersiz olmalıdır. Varsayılan olarak, UUID kullanılacaktır. Bir görünen ad özniteliğinin grubu göstermek için kullanılması amaçlandığından, bu dahili grup adı muhtemelen görünür olmayacaktır.",
    "Internal Groupname Attribute:" : "Dahili Grup Adı Özniteliği:",
