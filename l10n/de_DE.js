OC.L10N.register(
    ...
    "Settings verified, but more than one user was found. Only the first will be able to login. Consider a more narrow filter." : "Einstellungen überprüft, aber mehr als einen Nutzer gefunden. Nur der Erste kann sich anmelden. Verwenden Sie möglicherweise einen engeren Filter.",
    ...
    "Allows login against an email attribute. Mail and mailPrimaryAddress will be allowed. WARNING: Disabling login with email might require enabling strict login checking to be effective, please refer to ownCloud documentation for more details!" : "Erlaubt die Anmeldung gegen ein E-Mail-Attribut. Mail und mailPrimaryAddress werden zugelassen. WARNUNG: Die Deaktivierung der Anmeldung per E-Mail erfordert möglicherweise die Aktivierung von `strict login checking`, um wirksam zu sein. Weitere Einzelheiten finden Sie in der ownCloud-Dokumentation!",
    ...
    "Use StartTLS support" : "StartTLS-Unterstützung verwenden",
    "Enable StartTLS support (also known as LDAP over TLS) for the connection.  Note that this is different than LDAPS (LDAP over SSL) which doesn't need this checkbox checked. You'll need to import the LDAP server's certificate in your %s server." : "Aktivieren Sie die StartTLS-Unterstützung (auch bekannt als LDAP over TLS) für die Verbindung. Beachten Sie, dass sich dies von LDAPS (LDAP over SSL) unterscheidet, bei dem dieses Kontrollkästchen nicht aktiviert werden muss. Sie müssen das Zertifikat des LDAP-Servers in Ihren %s-Server importieren.",
    "User DN" : "Benutzer-DN",
    ...
    "Each attribute value is truncated to 191 characters" : "Jeder Attributwert wird auf 191 Zeichen gekürzt",
    ...
    "Group-Member algorithm" : "Gruppe-Mitglied-Algorithmus",
    "Algorithm to be used when searching for members in a group. In case of doubt, \"%s\" is the safe choice. Note that you're responsible of fulfilling the restrictions in order to use any algorithm; the app will behave erratically if the restrictions aren't fulfilled" : "Das ist der Algorithmus, um nach Teilnehmern einer Gruppe zu suchen. \"%s\" ist im Zweifel die sichere Wahl. Beachten Sie, dass Sie dafür verantwortlich sind, die Bedingungen zu erfüllen, um einen Algorithmus zu verwenden. Die App verhält sich unvorhersehbar, wenn die Einschränkungen nicht erfüllt sind.",
    "Scan the group looking for users. This is the basic method and works without restrictions. It supports nested groups" : "Scannen Sie die Gruppe nach Benutzern. Dies ist die grundlegende Methode und funktioniert ohne Einschränkungen. Verschachtelte Gruppen werden unterstützt.",
    "Search the members using the \"memberOf\" attribute. It requires support for the \"memberOf\" attribute, and it doesn't work with nested groups" : "Durchsuchen Sie die Mitglieder nach dem Attribut \"memberOf\". Das Attribut \"memberOf\" muss unterstützt werden, funktioniert aber nicht mit verschachtelten Gruppen.",
    "Search the members using the \"memberOf\" attribute in a recursive way. It requires support for the \"memberOf\" attribute and support for the \"LDAP_MATCHING_RULE_IN_CHAIN\" operator, it works with nested groups. Its main target is AD servers. NOTE: This algorithm only works with members explicitly listed in the group; AD's primary group, for example, won't be taken into account." : "Durchsuchen Sie die Mitglieder rekursiv mithilfe des Attributs „memberOf“. Das Attribut „memberOf“ und der Operator „LDAP_MATCHING_RULE_IN_CHAIN“ muss unterstützt werden. Verschachtelte Gruppen werden unterstützt. Das Hauptziel sind AD-Server. HINWEIS: Dieser Algorithmus funktioniert nur mit Mitgliedern, die explizit in einer Gruppe stehen. Die primäre Gruppe von AD wird beispielsweise nicht berücksichtigt.",
    ...
    "Leave empty for user's default quota. Otherwise, specify an LDAP/AD attribute." : "Ohne Eingabe wird der Standardwert für den Speicherplatz der Nutzer verwendet. Anderenfalls tragen Sie bitte ein LDAP/AD-Attribut ein.",
    "Quota Default" : "Standard-Kontingent",
    "Override default quota for LDAP users who do not have a quota set in the Quota Field." : "Standardwert für den Speicherplatz der LDAP-Nutzer, die keinen Speicherplatz im entsprechenden Feld angegeben haben.",
    "Email Field" : "E-Mail-Feld",
    "Set the user's email from their LDAP attribute. Leave it empty for default behaviour." : "Die E-Mail Adresse des Nutzers entsprechend dessen LDAP-Attribut festlegen. Für den Standardwert leer lassen.",
    ...
    "Exposed User Attributes" : "Benutzerattribute wurden bloßgelegt",
    "User attributes that other apps will be able to check" : "Benutzerattribute, die von anderen Anwendungen überprüft werden können",
    "Internal Username" : "Interner Benutzername",
    "By default the internal username will be created from the UUID attribute. It makes sure that the username is unique and characters do not need to be converted. The internal username has the restriction that only these characters are allowed: [ a-zA-Z0-9+_.@- ].  Other characters are replaced with their ASCII correspondence or simply omitted. On collisions a number will be added/increased. The internal username is used to identify a user internally. It is also the default name for the user home folder. It is also a part of remote URLs, for instance for all *DAV services. With this setting, the default behavior can be overridden. To do so, enter the user display name attribute in the following field. Leave it empty for default behavior. Changes will have effect only on newly mapped (added) LDAP users." : "Standardmäßig wird der interne Benutzername aus dem UUID-Attribut erzeugt. Dadurch wird sichergestellt, dass der Benutzername einzigartig ist und keine Zeichen umgewandelt werden müssen. Der interne Benutzername darf nur aus folgenden Zeichen bestehen: [ a-zA-Z0-9+_.@- ]. Andere Zeichen werden durch ihre ASCII-Entsprechung ersetzt oder ausgelassen. Sollte der Benutzername schon existieren, wird eine Zahl angehängt bzw. diese hochgezählt. Der interne Benutzername dient dazu, den Benutzer im System eindeutig zu identifizieren. Er ist außerdem die Standard-Bezeichnung für das Home-Verzeichnis des Benutzers. Er wird außerdem als Teil von Remote-URLs verwendet, z.B. für alle *DAV-Dienste. Das Standard-Verhalten kann überschrieben werden. Geben Sie dazu im folgenden Feld das Attribut für Benutzernamensanzeigen ein. Bleibt das Feld leer, gilt das Standard-Verhalten. Änderungen wirken sich lediglich bei neu hinzugefügten LDAP-Benutzern aus.",
    "Internal Username Attribute:" : "Attribut für internen Benutzernamen:",
    "Internal Groupname" : "Interner Gruppenname",
    "The internal groupname is used to uniquely identify the group. It has the same restrictions as the internal username, in particular, the group name must be immutable and unique. By default, the UUID will be used. This internal groupname won't likely by visible because a displayname attribute is intended to be used to show the group." : "Der interne Gruppenname identifiziert eine Gruppe eindeutig. Er unterliegt den gleichen Einschränkungen wie der interne Benutzername. Insbesondere muss der Gruppenname unveränderlich und eindeutig sein. Standardmäßig wird die UUID verwendet. Dieser interne Gruppenname ist normalerweise nicht sichtbar, da ein Anzeigename-Attribut dafür vorgesehen ist.",
    "Internal Groupname Attribute:" : "Attribut für internen Gruppennamen:",
