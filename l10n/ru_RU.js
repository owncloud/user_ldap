OC.L10N.register(
    ...
    "No data specified" : "Данные не указаны",
    ...
    "2nd User Display Name Field" : "Поле отображения имени 2-го пользователя",
    ...
    "Group-Member algorithm" : "Алгоритм членства в группе",
    "Algorithm to be used when searching for members in a group. In case of doubt, \"%s\" is the safe choice. Note that you're responsible of fulfilling the restrictions in order to use any algorithm; the app will behave erratically if the restrictions aren't fulfilled" : "Алгоритм, который используется для поиска членов группы. В случае сомнений, самый безопасный выбор — \"%s\". Учтите, что вы отвечаете за выполнение ограничений при использовании любого алгоритма; приложение может вести себя ошибочно, если ограничения не выполняются",
    "Scan the group looking for users. This is the basic method and works without restrictions. It supports nested groups" : "Полный просмотр группы при поиске пользователей. Это базовый метод и он работает без ограничений. Поддерживаются вложенные группы",
    "Search the members using the \"memberOf\" attribute. It requires support for the \"memberOf\" attribute, and it doesn't work with nested groups" : "Поиск членов через атрибут \"memberOf\". Требуется поддержка атрибута \"memberOf\", и это не работает со вложенными группами",
    "Search the members using the \"memberOf\" attribute in a recursive way. It requires support for the \"memberOf\" attribute and support for the \"LDAP_MATCHING_RULE_IN_CHAIN\" operator, it works with nested groups. Its main target is AD servers. NOTE: This algorithm only works with members explicitly listed in the group; AD's primary group, for example, won't be taken into account." : "Рекурсивный поиск членов группы с использованием атрибута «memberOf». Требуется поддержка атрибута «memberOf» и поддержка оператора «LDAP_MATCHING_RULE_IN_CHAIN», работает с вложенными группами. Основная цель - серверы AD. ПРИМЕЧАНИЕ: Этот алгоритм работает только с членами, явно указанными в группе; основная группа AD, например, не будет принята во внимание.",
    ...
    "When switched on, groups that contain groups are supported. (Only works if the group member attribute contains DNs.)" : "При включении, активируется поддержка групп, содержащих другие группы. (Работает только если атрибут член группы содержит DNs.)",
    ...
    "Exposed User Attributes" : "воздействию атрибутов пользователей",
    "User attributes that other apps will be able to check" : "Пользовательские атрибуты, которые могут проверять другие приложения",
    ...
    "Internal Groupname" : "Имя внутренней группы",
    "The internal groupname is used to uniquely identify the group. It has the same restrictions as the internal username, in particular, the group name must be immutable and unique. By default, the UUID will be used. This internal groupname won't likely by visible because a displayname attribute is intended to be used to show the group." : "Имя внутренней группы используется для уникальной идентификации группы. Оно имеет те же ограничения, что и внутреннее имя пользователя, в частности, имя группы должно быть неизменяемым и уникальным. По умолчанию используется UUID. Это внутреннее имя группы, скорее всего, не будет видно, поскольку атрибут displayname предназначен для отображения группы.",
    "Internal Groupname Attribute:" : "Внутреннее имя группы атрибутов:",
