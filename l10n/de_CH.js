OC.L10N.register(
    ...
    "Host" : "Host",
    "Port" : "Port",
    "You can omit the protocol, except you require SSL. Then start with ldaps://" : "Du kannst das Protokoll auslassen, ausser wenn Du SSL benötigst. Beginne dann mit ldaps://",
    ...
    "Manually enter LDAP filters (recommended for large directories)" : "LDAP-Filter manuell eingeben (empfohlen für grosse Verzeichnisse)",
    "Avoids automatic LDAP requests. Better for bigger setups, but requires some LDAP knowledge." : "Verhindert automatische LDAP-Anfragen. Besser geeignet für grössere Installationen, benötigt aber erweiterte LDAP-Kenntnisse.",
    ...
    "Not recommended, use it for testing only! If connection only works with this option, import the LDAP server's SSL certificate in your %s server." : "Nur für Testzwecke geeignet, sollte Standardmässig nicht verwendet werden. Falls die Verbindung nur mit dieser Option funktioniert, importiere das SSL-Zertifikat des LDAP-Servers in deinen %s Server.",
    ...
    "By default the internal username will be created from the UUID attribute. It makes sure that the username is unique and characters do not need to be converted. The internal username has the restriction that only these characters are allowed: [ a-zA-Z0-9+_.@- ].  Other characters are replaced with their ASCII correspondence or simply omitted. On collisions a number will be added/increased. The internal username is used to identify a user internally. It is also the default name for the user home folder. It is also a part of remote URLs, for instance for all *DAV services. With this setting, the default behavior can be overridden. To achieve a similar behavior as before ownCloud 5 enter the user display name attribute in the following field. Leave it empty for default behavior. Changes will have effect only on newly mapped (added) LDAP users." : "Standardmässig wird der interne Benutzername aus dem UUID-Attribut erstellt. Es stellt sicher, dass der Benutzername eindeutig ist und Zeichen nicht konvertiert werden müssen. Der interne Benutzername hat die Einschränkung, dass nur diese Zeichen erlaubt sind: [ a-zA-Z0-9+_.@- ]. Andere Zeichen werden durch ihre ASCII-Entsprechung ersetzt oder einfach weggelassen. Bei Kollisionen wird eine Zahl hinzugefügt/erhöht. Der interne Benutzername wird zur internen Identifizierung eines Benutzers verwendet. Er ist auch der Standardname für den Home-Ordner des Benutzers. Er ist auch ein Teil von Remote-URLs, zum Beispiel für alle *DAV-Dienste. Mit dieser Einstellung kann das Standardverhalten ausser Kraft gesetzt werden. Um ein ähnliches Verhalten wie vor ownCloud 5 zu erreichen, geben Sie das Attribut für den Anzeigenamen des Benutzers in das folgende Feld ein. Lassen Sie es für das Standardverhalten leer. Änderungen wirken sich nur auf neu zugeordnete (hinzugefügte) LDAP-Benutzer aus.",
