OC.L10N.register(
    ...
    "By default the internal username will be created from the UUID attribute. It makes sure that the username is unique and characters do not need to be converted. The internal username has the restriction that only these characters are allowed: [ a-zA-Z0-9+_.@- ].  Other characters are replaced with their ASCII correspondence or simply omitted. On collisions a number will be added/increased. The internal username is used to identify a user internally. It is also the default name for the user home folder. It is also a part of remote URLs, for instance for all *DAV services. With this setting, the default behavior can be overridden. To achieve a similar behavior as before ownCloud 5 enter the user display name attribute in the following field. Leave it empty for default behavior. Changes will have effect only on newly mapped (added) LDAP users." : "Por defecto el nombre de usuario interno sera creado con el atributo UUID. Esto asegura que el usuario es único y los caracteres no necesitan ser convertidos. El nombre de usuario interno tiene una restricción y solo los siguientes caracteres están permitidos: [ a-zA-Z0-9+_.@- ]. Otros caracteres son remplazados con su correspondiente en ASCII o simplemente omitidos. En caso de colisión, se le sumará un número. El nombre de usuario interno se usa para identificar al usuario internamente. También es el nombre por defecto de la carpeta de archivos del usuario. También es parte de las URLs remotas, por ejemplo para todos los servicios *DAV. Con esta opción, el comportamiento por defecto puede ser anulado. Para tener un comportamiento similar anterior a ownCloud 5 ingrese el atributo \"display name\" del usuario en el siguiente campo. Deje esto vacío para el comportamiento por defecto. Los cambios solo tendrán efecto en nuevos usuarios de LDAP mapeados (agregados).",
