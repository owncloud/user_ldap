OC.L10N.register(
    ...
    "Search the members using the \"memberOf\" attribute in a recursive way. It requires support for the \"memberOf\" attribute and support for the \"LDAP_MATCHING_RULE_IN_CHAIN\" operator, it works with nested groups. Its main target is AD servers" : "Поиск членов через атрибут \"memberOf\" рекурсивным образом. Требуется поддержка атрибута \"memberOf\" и поддержка оператора \"LDAP_MATCHING_RULE_IN_CHAIN\", работает со вложенными группами. Основная область применения — сервера AD.",
