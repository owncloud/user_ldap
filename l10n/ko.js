OC.L10N.register(
    ...
    "Settings verified, but more than one user was found. Only the first will be able to login. Consider a more narrow filter." : "설정을 찾았지만 한 명 이상의 사용자를 찾았습니다. 첫 번째 사용자만 로그인할 수 있습니다. 더 좁은 필터를 적용해 보십시오.",
    ...
    "Use StartTLS support" : "StartTLS 지원 사용",
    "Enable StartTLS support (also known as LDAP over TLS) for the connection.  Note that this is different than LDAPS (LDAP over SSL) which doesn't need this checkbox checked. You'll need to import the LDAP server's certificate in your %s server." : "연결에 대해 StartTLS(LDAP over TLS) 지원을 켭니다. LDAPS(LDAP over SSL)와는 다르며, LDAPS을 이용하려면 이 설정을 켜지 않아도 됩니다. %s 서버의 LDAP 인증서를 가져와야 합니다.",
    "User DN" : "사용자 DN",
    ...
    "%s access is limited to users meeting these criteria:" : "다음 조건을 만족하는 사용자로 %s 접근이 제한됩니다:",
    ...
    "timeout for all the ldap network operations, in seconds." : "모든 ldap 네트워크 작업에 대한 타임아웃을 초 단위로 설정합니다.",
    ...
    "Each attribute value is truncated to 191 characters" : "각각 속성 값은 191글자로 잘림",
    ...
    "Group-Member algorithm" : "그룹-구성원 알고리즘",
    "Algorithm to be used when searching for members in a group. In case of doubt, \"%s\" is the safe choice. Note that you're responsible of fulfilling the restrictions in order to use any algorithm; the app will behave erratically if the restrictions aren't fulfilled" : "알고리즘은 그룹 내 구성원을 검색할 때 쓰입니다. 확실하지 않을 때는 \"%s\" 설정을 선택하는 것이 안전합니다. 알고리즘을 사용하려면 제한 사항을 충족해야 하며, 이를 충족하지 않을 경우 앱이 제대로 동작하지 않을 수 있습니다.",
    "Scan the group looking for users. This is the basic method and works without restrictions. It supports nested groups" : "그룹을 스캔하여 사용자를 찾습니다. 이는 기본 방법이며 제한 없이 작동합니다. 중첩된 그룹을 지원합니다.",
    "Search the members using the \"memberOf\" attribute. It requires support for the \"memberOf\" attribute, and it doesn't work with nested groups" : "\"memberOf\" 속성을 사용하여 사용자를 찾습니다. \"memberOf\" 속성 지원이 필요하며, 중첩된 그룹을 지원하지 않습니다.",
    "Search the members using the \"memberOf\" attribute in a recursive way. It requires support for the \"memberOf\" attribute and support for the \"LDAP_MATCHING_RULE_IN_CHAIN\" operator, it works with nested groups. Its main target is AD servers. NOTE: This algorithm only works with members explicitly listed in the group; AD's primary group, for example, won't be taken into account." : "재귀적 방법으로 \"memberOf\" 속성이 있는 구성원을 검색하며, 중첩된 그룹에서 작동합니다. AD 서버를 위해 준비한 기능이며, \"memberOf\" 속성 및 \"LDAP_MATCHING_RULE_IN_CHAIN\" 연산자에 대한 지원이 필요합니다. 참고: 이 알고리즘은 그룹 내에 있는 구성원에 대해서만 작동하며, AD 기본 그룹 내에 있는 구성원은 고려되지 않습니다.",
